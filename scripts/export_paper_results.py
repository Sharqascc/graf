...
def main():
    ...
    if first_frame.exists():
        ...
        if graph_path.exists():
            ...
            ax.scatter(
                node_positions[:, 0], node_positions[:, 1], c="red", s=30, zorder=5
            )
            ax.set_title("First frame interaction graph")
            ax.axis("off")
            fig.savefig(
                out_dir / "spatial_graph_frame0.png", dpi=150, bbox_inches="tight"
            )
            plt.close(fig)
    ...
    if len(window_ds) > 0:
        ...
        for ax, fid in zip(axes, frames):
            ...
            ax.axis("off")
        fig.savefig(
            out_dir / "temporal_window_preview.png", dpi=150, bbox_inches="tight"
        )
        plt.close(fig)
