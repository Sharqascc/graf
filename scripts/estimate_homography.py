...
def main():
    parser = argparse.ArgumentParser(
        description="Estimate homography using RANSAC from calibration points"
    )
    parser.add_argument(
        "--input_config",
        required=True,
        help="YAML with calibration_points_pixel and calibration_points_world or calibration_points_latlon",
    )
    parser.add_argument(
        "--output_config", default=None, help="Output YAML (default: overwrite input)"
    )
    parser.add_argument(
        "--ransac_threshold",
        type=float,
        default=1.0,
        help="RANSAC reprojection threshold in pixels? or world units?",
    )
    ...
    if "calibration_points_pixel" not in cfg or "calibration_points_world" not in cfg:
        raise ValueError(
            "Config must contain calibration_points_pixel and calibration_points_world"
        )
