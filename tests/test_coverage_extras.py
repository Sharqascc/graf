...
# SSM
from graf.ssm.pet import compute_pet_from_conflict_zone
from graf.ssm.ttc import compute_ttc_constant_velocity

# Utils
from graf.utils.io import snapshot_environment
from graf.utils.pipeline_status import count_real_files
...
def test_build_pyg_graph_for_frame():
    actors = [
        {"track_id": 1, "actor_class": "car", "x_m": 0, "y_m": 0, "vx": 1, "vy": 0},
        {
            "track_id": 2,
...
def test_validate_feature_layout_missing_x():
    data = Data(
        edge_index=torch.empty((2, 0), dtype=torch.long), edge_attr=torch.empty((0, 15))
    )
    with pytest.raises(ValueError, match=r"Data\.x is missing"):
...
def test_pet_zone_overlap():
    traj1 = np.array([[0, 0], [1, 0]])
    traj2 = np.array([[0, 0], [1, 0]])
    t1 = np.array([0, 1])
    t2 = np.array([0.5, 1.5])
    res = compute_pet_from_conflict_zone(
        traj1, traj2, t1, t2, np.array([0.5, 0]), zone_radius=1.0
    )
...
def test_pet_one_never_enter():
    traj1 = np.array([[0, 0]])
    traj2 = np.array([[10, 10]])
    res = compute_pet_from_conflict_zone(
        traj1, traj2, np.array([0]), np.array([0]), np.array([0, 0])
    )
    assert res.status == "one_or_both_never_enter_zone"


def test_ttc_no_collision_min_sep():
    res = compute_ttc_constant_velocity(
        np.array([0, 0]), np.array([1, 0]), np.array([1, 10]), np.array([-1, 0])
    )
