...
def make_frame(video_id, frame_id, num_nodes=3, has_y=False):
    ...
    data = Data(x=x, pos=pos, edge_index=edge_index, edge_attr=edge_attr,
                track_ids=track_ids, actor_class_index=actor_class_index,
...
def test_missing_required_attribute_raises():
    bad = Data(pos=torch.rand(3,2), edge_index=torch.tensor([[0],[1]]), edge_attr=torch.rand(1,2), track_ids=torch.arange(3))
    with pytest.raises(ValueError, match="must contain x"):
        build_temporal_window_graph([bad])
    bad2 = Data(x=torch.rand(3,5), pos=torch.rand(3,2), edge_attr=torch.rand(1,2), track_ids=torch.arange(3))
    with pytest.raises(ValueError, match="edge_index"):
        build_temporal_window_graph([bad2])
    bad3 = Data(x=torch.rand(3,5), pos=torch.rand(3,2), edge_index=torch.tensor([[0],[1]]), track_ids=torch.arange(3))
    with pytest.raises(ValueError, match="edge_attr"):
        build_temporal_window_graph([bad3])
    bad4 = Data(x=torch.rand(3,5), pos=torch.rand(3,2), edge_index=torch.tensor([[0],[1]]), edge_attr=torch.rand(1,2))
