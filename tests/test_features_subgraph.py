...
def test_filter_subgraph_by_class_empty_data():
    data = Data(
        ...
        actor_class_index=torch.empty(0, dtype=torch.long),
        num_nodes=0,
    )
    out = filter_subgraph_by_class(data, target_class_idx=1)
...
def test_validate_feature_layout_missing_edge_index():
    data = make_data(num_nodes=3, num_edges=2)
    del data.edge_index
    with pytest.raises(ValueError, match=r"Data\.edge_index is missing"):
        validate_feature_layout(data)
...
def test_validate_feature_layout_missing_edge_attr():
    data = make_data(num_nodes=3, num_edges=2)
    del data.edge_attr
    with pytest.raises(ValueError, match=r"Data\.edge_attr is missing"):
        validate_feature_layout(data)
...
def test_validate_feature_layout_wrong_x_rank():
    data = make_data(num_nodes=3, num_edges=2)
    data.x = torch.rand(3)
    with pytest.raises(ValueError, match=r"Data\.x must be rank-2"):
        validate_feature_layout(data)
...
def test_validate_feature_layout_wrong_edge_index_shape():
    ...
    with pytest.raises(ValueError, match=r"Data\.edge_index must have shape"):
        validate_feature_layout(data)
